          ...
          rows.map((row) => (
            <div key={row.slug} className="cg-explore-entity-row" onClick={() => handleRowClick(row)}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1, flexShrink: 0 }}>
                {row.evDcFastNum > 0 && (
                  <span style={{ fontSize: 11, fontFamily: "var(--cg-font-mono)", color: "var(--color-text-heading)" }}>
                    {row.evDcFastNum} DC Fast
                  </span>
                )}
                {row.evLevel2EvseNum > 0 && (
                  <span style={{ fontSize: 10.5, fontFamily: "var(--cg-font-mono)", color: "var(--color-text-muted)" }}>
