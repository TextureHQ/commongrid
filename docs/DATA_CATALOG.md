...
#### gridstatus — Open Source ISO Data Library
| Field | Value |
|-------|-------|
| **Description** | Python library providing a standardized API to access raw electricity supply, demand, and pricing data from all major US/Canadian ISOs. Includes LMP data, fuel mix, load forecasts, ancillary services, and outage data. |
| **Source** | Grid Status (open source) |
| **URL** | https://www.gridstatus.io/ |
| **GitHub** | https://github.com/gridstatus/gridstatus |
| **Docs** | https://opensource.gridstatus.io/ |
| **Format** | Python library returning pandas DataFrames; hosted API also available |
| **Coverage** | CAISO, ERCOT, ISO-NE, MISO, NYISO, PJM, SPP, IESO (Ontario), AESO (Alberta), EIA |
| **Update Frequency** | Real-time (scrapes directly from ISO websites) |
| **License** | BSD-3-Clause (library); data subject to individual ISO terms |
| **Notes** | Excellent for getting standardized LMP/pricing data, load, generation mix, and forecasts across all ISOs through one interface. The hosted API at gridstatus.io provides production-ready cleaned data (separate product). The open-source library gives raw data access. |
...
| **Grid interconnection queue data** (standardized, real-time) | Understanding the future generation pipeline and grid congestion | LBNL compiles queue data but with lag. Individual ISO queue portals are messy and inconsistent. No open real-time standardized feed exists. | Use gridstatus library to scrape ISO queue data. Supplement with LBNL annual compilation. Consider building a standardized aggregation layer. |

#### 🟡 Medium Priority Gaps

| Gap | Why It Matters | Current State | Potential Approaches |
|-----|---------------|---------------|---------------------|
| **Wholesale market pricing nodes / LMP maps** (static reference data) | Mapping LMP pricing nodes to geographic locations enables spatial market analysis | ISO OASIS systems have this but it's not standardized across ISOs. gridstatus provides real-time LMP data but the underlying node geography varies by ISO. | Use gridstatus for LMP data; build node-to-geography mapping from individual ISO GIS files. CAISO, PJM, and MISO publish node location files. |
...
3. **Real-time data is getting better** — EIA-930 (hourly grid monitor), gridstatus (ISO market data), and emissions APIs (WattTime, Electricity Maps) provide increasingly good real-time coverage. But it requires API access and stitching together multiple sources.
