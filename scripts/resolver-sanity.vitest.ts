...
const RESOLVER_FIXTURES = [
  ...
  { name: 'Tri-State Electric Member Coop', state: 'CO', expectedId: 'xxxxx', expectedConfidence: 0.90 },
  ...
  { name: 'GMP', state: 'VT', expectedId: 'green-mountain-power-id', expectedConfidence: 0.75, description: 'domain-based' },
  { name: 'green mountain power', state: 'VT', expectedId: 'green-mountain-power-id', expectedConfidence: 0.95, description: 'exact name' },
  
  // Mixed case + punctuation (normalize expected)
  { name: 'Vermont Electric   Co-Op!', state: 'VT', expectedId: '19791', expectedConfidence: 0.95, description: 'normalized name' },
];
...
describe('M5: Resolver Sanity Fixture', () => {
  ...
  it('should resolve known co-ops with high confidence (>= 0.90)', async () => {
    ...
    expect(recall).toBeGreaterThanOrEqual(0.90);  // >= 90% recall
  });
  ...
  it('should publish precision/recall metrics for threshold decisions', () => {
    ...
    Recommended threshold: ${recall >= 0.95 ? '0.90' : recall >= 0.90 ? '0.85' : '0.80'}
    `);

    // Store metrics in a file for CI consumption
    expect(successfulMatches).toBeGreaterThanOrEqual(Math.ceil(totalTests * 0.90));
  });
  ...
  it('should demonstrate cascade order: override → exact → domain → fuzzy', async () => {
    ...
    expect(exactMatch.rows[0]?.contract?.confidence).toBeGreaterThanOrEqual(0.90);
