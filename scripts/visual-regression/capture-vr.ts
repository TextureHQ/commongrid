...
function createWorktree(baseRef: string): string {
	...
	// Install deps in worktree if package-lock exists
...
async function main() {
	...
	if (!opts.skipCapture) {
		try {
			...
			if (!opts.skipBase) {
				// Create worktree for base
				console.log(`🌳 Creating worktree for ${opts.baseRef}...`);
				worktreePath = createWorktree(opts.baseRef);
