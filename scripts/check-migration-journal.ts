...
function checkJournal() {
  ...
  try {
    journal = JSON.parse(fs.readFileSync(journalPath, "utf8")) as JournalData;
  } catch (error) {
    console.error(`Error reading _journal.json: ${error}`);
    process.exit(1);
  }

  const journalledTags = new Set(journal.entries.map((entry) => entry.tag));
  let errorCount = 0;
  ...
  for (const entry of journal.entries) {
    if (entry.when <= lastWhen) {
      console.error(
        `❌ Journal entry for ${entry.tag} has non-monotonic timestamp ${entry.when} (previous was ${lastWhen})`
      );
      errorCount++;
    }
    lastWhen = entry.when;
