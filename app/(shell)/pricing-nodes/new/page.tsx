...
export default function CreatePricingNodePage() {
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        ...
        setFieldsError(null);
        const res = await fetch("/api/v1/editable-fields/pricing_node");
        if (!res.ok) {
  ...
  useEffect(() => {
    if (formValues.name && typeof formValues.name === "string") {
      setEditSummary(`Added new pricing node: ${formValues.name}`);
    }
  }, [formValues.name]);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (json.auto_approved && json.data?.entitySlug) {
          router.push(`/pricing-nodes/${json.data.entitySlug}`);
        } else {
          // Otherwise, redirect to the list
          router.push("/pricing-nodes");
        }
      }, 2000);
    } catch (error) {
  ...
  if (!user) {
    return (
      <PageLayout>
        <PageLayout.Header title="Add New Pricing Node" />
        <PageLayout.Content>
          <div className="max-w-2xl mx-auto py-8">
            <div className="rounded-md bg-blue-50 p-4 border border-blue-200">
              <p className="text-sm font-medium text-blue-800">Sign in required</p>
              <p className="text-sm text-blue-700 mt-1">You need to be signed in to add a new pricing node.</p>
            </div>
          </div>
        </PageLayout.Content>
      </PageLayout>
            ...
            <>
              ...
              {submitSuccess && (
                <div className="rounded-md bg-green-50 p-4 border border-green-200">
                  <p className="text-sm font-medium text-green-800">Pricing node added!</p>
                  <p className="text-sm text-green-700 mt-1">
                    Your new pricing node has been submitted. Thank you for contributing to CommonGrid!
                  </p>
                </div>
              )}
