...
export default function CreateEVStationPage() {
  ...
  const [sourceType, setSourceType] = useState("government_db");
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        ...
        setFieldsError(null);
        const res = await fetch("/api/v1/editable-fields/ev_station");
        if (!res.ok) {
  ...
  // Pre-fill edit summary when station_name changes
  useEffect(() => {
    const stationName = formValues.stationName || formValues.station_name;
    if (stationName && typeof stationName === "string") {
      setEditSummary(`Added new EV charging station: ${stationName}`);
    }
  }, [formValues.stationName, formValues.station_name]);

  // Duplicate check when station name changes (debounced)
  useEffect(() => {
    const stationName = formValues.stationName || formValues.station_name;
    if (!stationName || typeof stationName !== "string" || stationName.trim().length < 3) {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`/api/v1/ev-stations?q=${encodeURIComponent(stationName.trim())}&limit=5`);
        if (res.ok) {
          const json = await res.json();
          const matches = json.data?.filter((s: { stationName: string }) =>
            s.stationName.toLowerCase().includes(stationName.toLowerCase())
          );
          if (matches && matches.length > 0) {
            setDuplicateWarning(
              `An EV station named "${matches[0].stationName}" already exists. Did you mean to edit it instead?`
            );
          } else {
            setDuplicateWarning(null);
          }
  ...
  const hasRequiredFields = !!(formValues.stationName || formValues.station_name);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (json.auto_approved && json.data?.entitySlug) {
          router.push(`/ev-charging/${json.data.entitySlug}`);
        } else {
  ...
  if (!user) {
    return (
      <PageLayout>
        <PageLayout.Header title="Add New EV Charging Station" />
        <PageLayout.Content>
          <div className="max-w-2xl mx-auto py-8">
            <div className="rounded-md bg-blue-50 p-4 border border-blue-200">
              <p className="text-sm font-medium text-blue-800">Sign in required</p>
              <p className="text-sm text-blue-700 mt-1">You need to be signed in to add a new EV charging station.</p>
            </div>
          </div>
        </PageLayout.Content>
      </PageLayout>
            ...
            <>
              ...
              {submitSuccess && (
                <div className="rounded-md bg-green-50 p-4 border border-green-200">
                  <p className="text-sm font-medium text-green-800">EV station added!</p>
                  <p className="text-sm text-green-700 mt-1">
                    Your new EV charging station has been submitted. Thank you for contributing to CommonGrid!
                  </p>
                </div>
              )}
